 ...
 */
	...
	function is_date($date, $format = 'd-m-Y')
	{
		...
		$format = 'Y-m-d G:i:s-03';
	    $is_date = DateTime::createFromFormat($format, $date);
		if ($is_date) {
			return $format;
		}
	...
	function h($text, $double = true, $charset = null) {
		if ($format = is_date($text)) {
			$text = DateTime::createFromFormat($format, $text);
			$text = $text->format('d/m/Y');
		}
		elseif (is_bool($text)) {
// 			$ret = '<input type="checkbox" readonly="readonly" onclick="return false;" ';
// 			if ($text == true) {
// 				$ret .= ' checked="checked" ';
// 			}
// 			$ret .= '/>';
			...
			if($text == true) {
				$ret = "<img src='". Router::url('/') ."img/verde.png' title='Si' alt='Si' class='checklist' />";
			} else {
				$ret = "<img src='". Router::url('/') ."img/rojo.png' title='No' alt='No' class='checklist' />";				
			}
			return $ret;
 ...
 */
	function __($singular, $args = null) {
		...
		// Si a lo ultimo dice " Id" se lo saco porque no me gusta
		if (strlen($singular) - strpos($singular, ' Id') == 3) {
			$singular = str_replace(' Id', '', $singular);
		}
...
/**
 * Transforma un texto a un dni agregandole los puntos cada 3 numeros
 *
 * @param unknown $text
 *
 * @return Integer
...
function dni($text) {
	...
	// upside down
	$alreves = '';
	for ($int = strlen($text)-1; $int >= 0; $int--) {
		$i++;
		$alreves .= $text[$int];

		if (!is_numeric($text[$int]) || !isset($text[$int-1]) || !is_numeric($text[$int-1])) {
			$i=0;
		}
	...
	$ret = '';
	for ($int = strlen($alreves)-1; $int >= 0; $int--) {
		$ret .= $alreves[$int];
	}
	return $ret;
}