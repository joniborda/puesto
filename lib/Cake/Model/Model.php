 ...
 */
	public function __get($name) {
		if ($name === 'displayField') {
			return $this->displayField = $this->hasField(array('descripcion', 'title', 'name', $this->primaryKey));
		}
		if ($name === 'tablePrefix') {
			$this->setDataSource();
				...
				if (!empty($newValues)) {
					if ($keepExisting && !empty($links)) {
						foreach ($links as $link) {
							...
							$oldJoin = $link[$join][substr($this->hasAndBelongsToMany[$assoc]['associationForeignKey'], 0, 36)];
