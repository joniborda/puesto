 ...
 */
	protected function _interactive() {
		...
		$actions = '';
		$wannaUseSession = 'y';
		$wannaBakeAdminCrud = 'y';
		$useDynamicScaffold = 'n';
		$wannaBakeCrud = 'y';

		$question[] = __d('cake_console', "Quiere crear un controller altervativo?");
		if (file_exists($this->path . $controllerName . 'Controller.php')) {
			$question[] = __d('cake_console', "Warning: Quiere sobreescribir el %sController.", $controllerName);
		}
		...
		if (strtolower($doItInteractive) === 'y') {
			...
			$useDynamicScaffold = $this->in(
				__d('cake_console', "Quiere usar la estructura dinamica?"), array('y', 'n'), 'n'
			);
		...
		$useSearch = $this->in(
				__d('cake_console', "Would you like to use Search plugin?"), array('y', 'n'), 'y'
		);
		
		if (strtolower($useSearch) === 'y') {
			$wannaSearch = 'y';
			$components[] = 'Search.Prg';
		}
		
		if (strtolower($wannaBakeCrud) === 'y') {
			$actions .= $this->bakeActions($controllerName, null, strtolower($wannaUseSession) === 'y', strtolower($wannaSearch) === 'y');
		}
		if (strtolower($wannaBakeAdminCrud) === 'y') {
			$admin = $this->Project->getPrefix();
			$actions .= $this->bakeActions($controllerName, $admin, strtolower($wannaUseSession) === 'y', strtolower($wannaSearch) === 'y');
		}
 ...
 */
	protected function _askAboutMethods() {
		$wannaBakeCrud = $this->in(
			__d('cake_console', "Quiere crear la clase con los basicos metodos \n(index(), add(), view(), edit())?"),
			array('y', 'n'), 'n'
		);
		$wannaBakeAdminCrud = $this->in(
			__d('cake_console', "Quiere crear la clase con los metodos admin_?"),
			array('y', 'n'), 'y'
		);
 ...
 */
	public function bakeActions($controllerName, $admin = null, $wannaUseSession = true, $wannaSearch = true) {
		...
		$this->Template->set(compact(
			...
			'displayField', 'primaryKey', 'wannaSearch'
		));
