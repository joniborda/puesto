 ...
 */
	protected function _interactive() {
		...
		if ($knownToExist) {
			$displayField = $tempModel->hasField(array('descripcion', 'name', 'title'));
			
			if (!$displayField) {
				
				$virtualFields = $this->findVirtualField($tempModel->schema());
				
				if ($virtualFields) {
					$displayField = 'virtual';
				} else {
			...
			$wannaDoValidation = $this->in($prompt, array('y', 'n'), 'n');
			if (array_search($useTable, $this->_tables) !== false && strtolower($wannaDoValidation) === 'y') {
				$validate = $this->doValidation($tempModel);
			}
			
			$prompt = __d('cake_console', "Would you like to use Search Plugin \nfor the fields in your model?");
				
			$wannaDoSearch = $this->in($prompt, array('y', 'n'), 'y');
			if (array_search($useTable, $this->_tables) !== false && strtolower($wannaDoSearch) === 'y') {
				$filterArgs = $this->doSearch($tempModel);
			}
		...
		if (strtolower($looksGood) === 'y') {
			$vars = compact('associations', 'validate', 'primaryKey', 'useTable', 'displayField', 'virtualFields','filterArgs');
 ...
 */
	public function findDisplayField($fields) {
		...
		$continue = $this->in($prompt, array('y', 'n'), 'y');
		if (strtolower($continue) === 'n') {
			return false;
		}
	...
	public function findVirtualField($fields) {
		$fieldNames = array_keys($fields);
		$prompt = __d('cake_console', "Queres concatenar un virtualField con dos campos?");
		$continue = $this->in($prompt, array('y', 'n'), 'n');
		if (strtolower($continue) === 'n') {
			return false;
		}
		...
		$prompt = __d('cake_console', 'Choose a field from the options above:');
		$choice = $this->inOptions($fieldNames, $prompt);
		$ret = $fieldNames[$choice];
		
		$prompt = __d('cake_console', "Ahora el otro campo");
		$continue = $this->in($prompt, array('y', 'n'), 'y');
		if (strtolower($continue) === 'n') {
			return false;
		}
		...
		$prompt = __d('cake_console', 'Choose a field from the options above:');
		$choice = $this->inOptions($fieldNames, $prompt);
		$ret1 = $fieldNames[$choice];
		
		return array($ret,$ret1);
 ...
 */
	...
	/**
	 * Handles Generation and user interaction for creating search.
	 *
	 * @param Model $model Model to have searching generated for.
	 * @return array $search Array of user selected validations.
	 */
	public function doSearch($model) {
		if (!is_object($model)) {
			return false;
		}
		$fields = $model->schema();
 ...
 */
	public function findBelongsTo(Model $model, $associations) {
		...
		foreach ($fieldNames as $fieldName) {
			...
			$field_sin_id = str_replace('_id', '', $fieldName);

			foreach ($this->_tables as $tabla) {
				if ($field_sin_id == $tabla) {
					$offset = strlen($fieldName);
				}
			}
	...
	public function findHasOneAndMany(Model $model, $associations) {
		...
		foreach ($this->_tables as $otherTable) {
			...
			foreach ($tempFieldNames as $fieldName) {
				$assoc = false;
				if ($fieldName != $model->primaryKey && ($fieldName == $foreignKey || $fieldName == $this->_controllerPath($model->name))) {
					$assoc = array(
 ...
 */
	public function bake($name, $data = array()) {
		if (is_object($name)) {
			...
			$data['filterArgs'] = $name->filterArgs;
		} else {
			$data['name'] = $name;
		}
		$defaults = array(
